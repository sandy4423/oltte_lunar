    ...
    <Card className="mb-6 border-orange-200">
      ...
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {Object.entries(pendingDeliveryItems).map(([key, entry]) => (
            <div key={key} className="bg-orange-50 border border-orange-200 rounded-lg p-3 text-center">
              <p className="text-xs text-gray-600 mb-1">{entry.label}</p>
              <p className="text-2xl font-bold text-orange-600">{entry.qty}개</p>
            </div>
          ))}
        </div>
      </CardContent>
